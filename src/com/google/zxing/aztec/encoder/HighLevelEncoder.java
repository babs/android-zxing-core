 ...
 * 
 * It uses a dynamic algorithm. For each prefix of the string, it determines a
 * set of encodings that could lead to this prefix. We repeatedly add a
...
public final class HighLevelEncoder {
	...
	static final String[] MODE_NAMES = { "UPPER", "LOWER", "DIGIT", "MIXED",
			"PUNCT" };
	...
	// shown
	static final int[][] SHIFT_TABLE = new int[6][6]; // mode shift codes, per
														// table
	static {
	...
	// the "result" list.
	private void updateStateForChar(State state, int index,
		...
		for (int mode = 0; mode <= MODE_PUNCT; mode++) {
			...
			if (charInMode > 0) {
				...
				if (!charInCurrentTable || mode == state.getMode()
						|| mode == MODE_DIGIT) {
					// If the character is in the current table, we don't want
					// to latch to
					// any other mode except possibly digit (which uses only 4
					// bits). Any
					// other latch would be equally successful *after* this
					// character, and
					// so wouldn't save any bits.
					State latch_state = stateNoBinary.latchAndAppend(mode,
							charInMode);
				...
				if (!charInCurrentTable
					...
					// It never makes sense to temporarily shift to another mode
					// if the
					// character exists in the current mode. That can never save
					// bits.
					State shift_state = stateNoBinary.shiftAndAppend(mode,
							charInMode);
		...
		if (state.getBinaryShiftByteCount() > 0
			...
			// It's never worthwhile to go into binary shift mode if you're not
			// already
			// in binary shift mode, and the character exists in your current
			// mode.
	...
	private static void updateStateForPair(State state, int index,
			int pairCode, Collection<State> result) {
		...
		if (pairCode == 3 || pairCode == 4) {
			// both characters are in DIGITS. Sometimes better to just add two
			// digits
			State digit_state = stateNoBinary.latchAndAppend(MODE_DIGIT,
					16 - pairCode) // period or comma in DIGIT
