...
public final class Encoder {
	...
	public static final int DEFAULT_EC_PERCENT = 33; // default minimal
														// percentage of error
														// check words
	...
	private static final int[] WORD_SIZE = { 4, 6, 6, 8, 8, 8, 8, 8, 8, 10, 10,
	 ...
	 *            input data string
	 * @param minECCPercent
	 *            minimal percentage of error check words (According to ISO/IEC
	 *            24778:2008, a minimum of 23% + 3 words is recommended)
	...
	public static AztecCode encode(byte[] data, int minECCPercent,
		...
		if (userSpecifiedLayers != DEFAULT_AZTEC_LAYERS) {
			...
			int usableBitsInLayers = totalBitsInLayer
			...
			if (compact && stuffedBits.getSize() > wordSize * 64) {
				// Compact format only allows 64 data words, though C4 can hold
			...
			// Compact2, Compact3,
			// Compact4, Normal4,... Normal(i) for i < 4 isn't typically used
			// since Compact(i+1)
			...
			for (int i = 0;; i++) {
				...
				// [Re]stuff the bits if this is the first opportunity, or if
				// the
		...
		BitArray messageBits = generateCheckWords(stuffedBits,
				totalBitsInLayer, wordSize);
			...
			for (int j = 0; j < rowSize; j++) {
				...
				for (int k = 0; k < 2; k++) {
					if (messageBits.get(rowOffset + columnOffset + k)) {
						matrix.set(alignmentMap[i * 2 + k], alignmentMap[i * 2
								+ j]);
					}
					if (messageBits.get(rowOffset + rowSize * 2 + columnOffset
							+ k)) {
						matrix.set(alignmentMap[i * 2 + j],
								alignmentMap[baseMatrixSize - 1 - i * 2 - k]);
					}
					if (messageBits.get(rowOffset + rowSize * 4 + columnOffset
							+ k)) {
						matrix.set(
								alignmentMap[baseMatrixSize - 1 - i * 2 - k],
								alignmentMap[baseMatrixSize - 1 - i * 2 - j]);
					}
					if (messageBits.get(rowOffset + rowSize * 6 + columnOffset
							+ k)) {
						matrix.set(
								alignmentMap[baseMatrixSize - 1 - i * 2 - j],
								alignmentMap[i * 2 + k]);
	...
	static BitArray generateModeMessage(boolean compact, int layers,
	...
	private static void drawModeMessage(BitMatrix matrix, boolean compact,
			int matrixSize, BitArray modeMessage) {
		int center = matrixSize / 2;
		if (compact) {
			for (int i = 0; i < 7; i++) {
	...
	private static int[] bitsToWords(BitArray stuffedBits, int wordSize,
			int totalWords) {
		...
		for (i = 0, n = stuffedBits.getSize() / wordSize; i < n; i++) {
			...
			for (int j = 0; j < wordSize; j++) {
				value |= stuffedBits.get(i * wordSize + j) ? (1 << wordSize - j
						- 1) : 0;
			}
			message[i] = value;
		}
