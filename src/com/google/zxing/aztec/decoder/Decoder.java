...
public final class Decoder {
	...
	boolean[] extractBits(BitMatrix matrix) {
		...
		for (int i = 0, rowOffset = 0; i < layers; i++) {
			...
			int low = i * 2;
			// The bottom-right most point of this layer is <high, high> (not
			// including alignment lines)
			int high = baseMatrixSize - 1 - low;
			// We pull bits from the two 2 x rowSize columns and two rowSize x 2
			// rows
			for (int j = 0; j < rowSize; j++) {
				...
				for (int k = 0; k < 2; k++) {
					...
					rawbits[rowOffset + 2 * rowSize + columnOffset + k] = matrix
							.get(alignmentMap[low + j], alignmentMap[high - k]);
					// right column
					rawbits[rowOffset + 4 * rowSize + columnOffset + k] = matrix
							.get(alignmentMap[high - k], alignmentMap[high - j]);
					// top row
					rawbits[rowOffset + 6 * rowSize + columnOffset + k] = matrix
							.get(alignmentMap[high - j], alignmentMap[low + k]);
				}
			}
			rowOffset += rowSize * 8;
		}
