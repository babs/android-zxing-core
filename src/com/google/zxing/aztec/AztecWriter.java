...
public final class AztecWriter implements Writer {
	...
	public BitMatrix encode(String contents, BarcodeFormat format, int width,
		...
		String charset = hints == null ? null : (String) hints
				.get(EncodeHintType.CHARACTER_SET);
		Number eccPercent = hints == null ? null : (Number) hints
				.get(EncodeHintType.ERROR_CORRECTION);
		Number layers = hints == null ? null : (Number) hints
	...
	private static BitMatrix encode(String contents, BarcodeFormat format,
			int width, int height, Charset charset, int eccPercent, int layers) {
	...
	private static BitMatrix renderResult(AztecCode code, int width, int height) {
		...
		int multiple = Math.min(outputWidth / inputWidth, outputHeight
				/ inputHeight);
