...
public final class QRCodeMultiReader extends QRCodeReader implements
	...
	private static List<Result> processStructuredAppend(List<Result> results) {
		...
		for (Result saResult : saResults) {
			...
			if (saResult.getResultMetadata().containsKey(
					ResultMetadataType.BYTE_SEGMENTS)) {
				@SuppressWarnings("unchecked")
				Iterable<byte[]> byteSegments = (Iterable<byte[]>) saResult
						.getResultMetadata().get(
								ResultMetadataType.BYTE_SEGMENTS);
		...
		for (Result saResult : saResults) {
			System.arraycopy(saResult.getRawBytes(), 0, newRawBytes,
			...
			if (saResult.getResultMetadata().containsKey(
				...
				Iterable<byte[]> byteSegments = (Iterable<byte[]>) saResult
						.getResultMetadata().get(
								ResultMetadataType.BYTE_SEGMENTS);
				for (byte[] segment : byteSegments) {
					System.arraycopy(segment, 0, newByteSegment,
