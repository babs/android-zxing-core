...
final class MultiFinderPatternFinder extends FinderPatternFinder {
	 ...
	 */
	private static final class ModuleSizeComparator implements
			Comparator<FinderPattern>, Serializable {
		@Override
		public int compare(FinderPattern center1, FinderPattern center2) {
			float value = center2.getEstimatedModuleSize()
					- center1.getEstimatedModuleSize();
	...
	private FinderPattern[][] selectMutipleBestPatterns()
		...
		if (size == 3) {
			return new FinderPattern[][] { new FinderPattern[] {
					possibleCenters.get(0), possibleCenters.get(1),
		...
		for (int i1 = 0; i1 < (size - 2); i1++) {
			...
			for (int i2 = i1 + 1; i2 < (size - 1); i2++) {
				...
				for (int i3 = i2 + 1; i3 < size; i3++) {
					...
					// Check the sizes
					float estimatedModuleCount = (dA + dB)
							/ (p1.getEstimatedModuleSize() * 2.0f);
