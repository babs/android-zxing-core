...
final class PDF417HighLevelEncoder {
	 ...
	 */
	private static final byte[] TEXT_MIXED_RAW = { 48, 49, 50, 51, 52, 53, 54,
	 ...
	 */
	private static final byte[] TEXT_PUNCTUATION_RAW = { 59, 60, 62, 64, 91,
	...
	private static final List<String> DEFAULT_ENCODING_NAMES = Arrays.asList(
			"Cp437", "IBM437");
	 ...
	 * @param encoding
	 *            character encoding used to encode in default or byte
	 *            compaction or {@code null} for default / not applicable
	 * @return the encoded message (the char values range from 0 to 928)
	 */
	static String encodeHighLevel(String msg, Compaction compaction,
	...
	private static byte[] toBytes(String msg, Charset encoding)
			throws WriterException {
		// Defer instantiating default Charset until needed, since it may be for
		// an unsupported
		// encoding. For example the default of Cp437 doesn't seem to exist on
		// Android.
		if (encoding == null) {
			...
			if (encoding == null) {
				throw new WriterException("No support for any encoding: "
	 ...
	 */
	private static int encodeText(CharSequence msg, int startpos, int count,
	 ...
	 */
	private static int determineConsecutiveDigitCount(CharSequence msg,
			int startpos) {
		int count = 0;
		int len = msg.length();
		int idx = startpos;
	 ...
	 * @return the requested character count
	 */
	private static int determineConsecutiveTextCount(CharSequence msg,
			int startpos) {
		int len = msg.length();
		int idx = startpos;
		while (idx < len) {
	 ...
	 * @return the requested character count
	 */
	private static int determineConsecutiveBinaryCount(CharSequence msg,
			byte[] bytes, int startpos) throws WriterException {
		int len = msg.length();
		int idx = startpos;
		while (idx < len) {
			char ch = msg.charAt(idx);
