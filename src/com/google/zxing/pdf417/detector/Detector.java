...
public final class Detector {
	...
	// if we set the value too low, then we don't detect the correct height of
	// the bar if the start patterns are damaged.
	// if we set the value too high, then we might detect the start pattern from
	// a neighbor barcode.
	private static final int SKIPPED_ROW_COUNT_MAX = 25;
	// A PDF471 barcode should have at least 3 rows, with each row being >= 3
	// times the module width. Therefore it should be at least
	// 9 pixels tall. To be conservative, we use about half the size to ensure
	// we don't miss it.
	 ...
	 * @param multiple
	 *            if true, then the image is searched for multiple codes. If
	 *            false, then at most one code will be found and returned
	 * @return {@link PDF417DetectorResult} encapsulating results of detecting a
	 *         PDF417 code
	...
	public static PDF417DetectorResult detect(BinaryBitmap image,
			...
			throws NotFoundException {
		// TODO detection improvement, tryHarder could try several different
		// luminance thresholds/blackpoints or even
	 ...
	 * @return List of ResultPoint arrays containing the coordinates of found
	 *         barcodes
	 */
	private static List<ResultPoint[]> detect(boolean multiple,
		...
		while (row < bitMatrix.getHeight()) {
			...
			if (vertices[0] == null && vertices[3] == null) {
				...
				// we didn't find a barcode starting at the given column and
				// row. Try again from the first column and slightly
			...
			// search for the next barcode after the
	 ...
	 * Start and Stop patterns as locators.
	 * 
	 * @param matrix
	 ...
	 */
	private static ResultPoint[] findVertices(BitMatrix matrix, int startRow,
			int startColumn) {
		...
		copyToResult(
				result,
				findRowsWithPattern(matrix, height, width, startRow,
						startColumn, START_PATTERN), INDEXES_START_PATTERN);
		...
		copyToResult(
				result,
				findRowsWithPattern(matrix, height, width, startRow,
						startColumn, STOP_PATTERN), INDEXES_STOP_PATTERN);
		return result;
	}

	private static void copyToResult(ResultPoint[] result,
			ResultPoint[] tmpResult, int[] destinationIndexes) {
	...
	private static ResultPoint[] findRowsWithPattern(BitMatrix matrix,
			int height, int width, int startRow, int startColumn, int[] pattern) {
		ResultPoint[] result = new ResultPoint[4];
		boolean found = false;
		int[] counters = new int[pattern.length];
		for (; startRow < height; startRow += ROW_STEP) {
			int[] loc = findGuardPattern(matrix, startColumn, startRow, width,
					false, pattern, counters);
			if (loc != null) {
				while (startRow > 0) {
					int[] previousRowLoc = findGuardPattern(matrix,
							startColumn, --startRow, width, false, pattern,
							counters);
		...
		if (found) {
			...
			int[] previousRowLoc = { (int) result[0].getX(),
					(int) result[1].getX() };
			for (; stopRow < height; stopRow++) {
				int[] loc = findGuardPattern(matrix, previousRowLoc[0],
						stopRow, width, false, pattern, counters);
				// a found pattern is only considered to belong to the same
				// barcode if the start and end positions
				// don't differ too much. Pattern drift should be not bigger
				// than two for consecutive rows. With
				// a higher number of skipped rows drift could be larger. To
				// keep it simple for now, we allow a slightly
	...
	private static int[] findGuardPattern(BitMatrix matrix, int column,
		...
		// left, but only for MAX_PIXEL_DRIFT pixels
