...
final class DetectionResultRowIndicatorColumn extends DetectionResultColumn {
	...
	// TODO maybe we should add missing codewords to store the correct row
	// number to make
	// finding row numbers for other columns easier
	// use row height count to make detection of invalid row numbers more
	// reliable
	int adjustCompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodeMetadata) {
		...
		ResultPoint top = isLeft ? boundingBox.getTopLeft() : boundingBox
				.getTopRight();
		ResultPoint bottom = isLeft ? boundingBox.getBottomLeft() : boundingBox
				.getBottomRight();
		int firstRow = imageRowToCodewordIndex((int) top.getY());
		int lastRow = imageRowToCodewordIndex((int) bottom.getY());
		// We need to be careful using the average row height. Barcode could be
		// skewed so that we have smaller and
		// taller rows
		...
		for (int codewordsRow = firstRow; codewordsRow < lastRow; codewordsRow++) {
			...
			// float expectedRowNumber = (codewordsRow - firstRow) /
			// averageRowHeight;
			// if (Math.abs(codeword.getRowNumber() - expectedRowNumber) > 2) {
			// SimpleLog.log(LEVEL.WARNING,
			// "Removing codeword, rowNumberSkew too high, codeword[" +
			// codewordsRow + "]: Expected Row: " +
			// expectedRowNumber + ", RealRow: " + codeword.getRowNumber() +
			// ", value: " + codeword.getValue());
			// codewords[codewordsRow] = null;
			// }

			int rowDifference = codeword.getRowNumber() - barcodeRow;

			// TODO improve handling with case where first row indicator doesn't
			// start with 0
			...
			if (rowDifference == 0) {
				...
				for (int i = 1; i <= checkedRows && !closePreviousCodewordFound; i++) {
					// there must be (height * rowDifference) number of
					// codewords missing. For now we assume height = 1.
	...
	BarcodeMetadata getBarcodeMetadata() {
			...
			switch (codewordRowNumber % 3) {
				case 0:
					barcodeRowCountUpperPart
							.setValue(rowIndicatorValue * 3 + 1);
					break;
				case 1:
		...
		if ((barcodeColumnCount.getValue().length == 0)
				...
				|| barcodeRowCountUpperPart.getValue()[0]
						+ barcodeRowCountLowerPart.getValue()[0] < PDF417Common.MIN_ROWS_IN_BARCODE
				|| barcodeRowCountUpperPart.getValue()[0]
						+ barcodeRowCountLowerPart.getValue()[0] > PDF417Common.MAX_ROWS_IN_BARCODE) {
			return null;
		}
	...
	private void removeIncorrectCodewords(Codeword[] codewords,
		...
		// TODO Maybe we should keep the incorrect codewords for the start and
		// end positions?
		for (int codewordRow = 0; codewordRow < codewords.length; codewordRow++) {
			...
			switch (codewordRowNumber % 3) {
				case 0:
					...
					break;
				case 1:
					if (rowIndicatorValue / 3 != barcodeMetadata
					...
					break;
				case 2:
					if (rowIndicatorValue + 1 != barcodeMetadata
							.getColumnCount()) {
						codewords[codewordRow] = null;
					}
					break;
