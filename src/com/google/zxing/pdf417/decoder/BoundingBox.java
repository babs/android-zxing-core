...
final class BoundingBox {
	...
	BoundingBox(BitMatrix image, ResultPoint topLeft, ResultPoint bottomLeft,
	...
	private void init(BitMatrix image, ResultPoint topLeft,
	...
	static BoundingBox merge(BoundingBox leftBox, BoundingBox rightBox)
		...
		return new BoundingBox(leftBox.image, leftBox.topLeft,
				leftBox.bottomLeft, rightBox.topRight, rightBox.bottomRight);
	}

	BoundingBox addMissingRows(int missingStartRows, int missingEndRows,
			boolean isLeft) throws NotFoundException {
		...
		calculateMinMaxValues();
		return new BoundingBox(image, newTopLeft, newBottomLeft, newTopRight,
				newBottomRight);
	}
	...
	private void calculateMinMaxValues() {
		if (topLeft == null) {
			...
			bottomRight = new ResultPoint(image.getWidth() - 1,
					bottomLeft.getY());
		}
