...
final class DecodedBitStreamParser {
	...
	/**
	 * Table containing values for the exponent of 900. This is used in the
	 * numeric compaction decode algorithm.
	 */
	...
	static DecoderResult decode(int[] codewords, String ecLevel)
		...
		while (codeIndex < codewords[0]) {
			switch (code) {
				...
				case MODE_SHIFT_TO_BYTE_COMPACTION_MODE:
					codeIndex = byteCompaction(code, codewords, codeIndex,
							result);
					break;
				...
				case BEGIN_MACRO_PDF417_CONTROL_BLOCK:
					codeIndex = decodeMacroBlock(codewords, codeIndex,
							resultMetadata);
					break;
				...
				default:
					// Default to text compaction. During testing numerous
					// barcodes
		...
		DecoderResult decoderResult = new DecoderResult(null,
				result.toString(), null, ecLevel);
		decoderResult.setOther(resultMetadata);
		return decoderResult;
	}

	private static int decodeMacroBlock(int[] codewords, int codeIndex,
		...
		resultMetadata.setSegmentIndex(Integer.parseInt(decodeBase900toBase10(
			...
			if (code < TEXT_COMPACTION_MODE_LATCH) {
				...
				switch (code) {
					...
					case MODE_SHIFT_TO_BYTE_COMPACTION_MODE:
						...
						// switch from Text Compaction mode to Byte Compaction
						// mode.
				...
				case PUNCT_SHIFT:
					...
					if (subModeCh < PAL) {
						...
						if (subModeCh == PAL) {
							...
							// PS before Shift-to-Byte is used as a padding
							// character,
	...
	private static int byteCompaction(int mode, int[] codewords, int codeIndex,
		...
		if (mode == BYTE_COMPACTION_MODE_LATCH) {
			...
			while ((codeIndex < codewords[0]) && !end) {
				...
				// perhaps it should be ok to check only nextCode >=
			...
			// if the end of all codewords is reached the last codeword needs to
			// be added
	...
	private static String decodeBase900toBase10(int[] codewords, int count)
		...
		for (int i = 0; i < count; i++) {
			result = result.add(EXP900[count - i - 1].multiply(BigInteger
					.valueOf(codewords[i])));
		}
