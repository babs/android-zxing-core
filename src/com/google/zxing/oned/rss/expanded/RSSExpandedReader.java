...
public final class RSSExpandedReader extends AbstractRSSReader {
	...
	// FINDER_PATTERN_SEQUENCES[FINDER_PATTERN_SEQUENCES.length - 1].length;
	...
	public Result decodeRow(int rowNumber, BitArray row,
			...
			FormatException {
		// Rows can start with even pattern in case in prev rows there where odd
		// number of patters.
		// So lets try twice
		this.pairs.clear();
	...
	List<ExpandedPair> decodeRow2pairs(int rowNumber, BitArray row)
		...
		try {
			while (true) {
				ExpandedPair nextPair = retrieveNextPair(row, this.pairs,
						rowNumber);
		...
		if (tryStackedDecode) {
			// When the image is 180-rotated, then rows are sorted in wrong
			// dirrection.
	...
	private List<ExpandedPair> checkRows(boolean reverse) {
		...
		// We use recursive algorithm with pure complexity and don't want it to
		// take forever
		// Stacked barcode can have up to 11 rows, so 25 seems resonable enough
		if (this.rows.size() > 25) {
			this.rows.clear(); // We will never have a chance to get result, so
								// clear it
			return null;
		}
	...
	private List<ExpandedPair> checkRows(List<ExpandedRow> collectedRows,
			int currentRow) throws NotFoundException {
	...
	private void storeRow(int rowNumber, boolean wasReversed) {
		// Discard if duplicate above or below; otherwise insert in order by row
		// number.
		int insertPos = 0;
	...
	ExpandedPair retrieveNextPair(BitArray row,
		...
		// When stacked symbol is split over multiple rows, there's no way to
		// guess if this pair can be last or not.
		...
		if (!previousPairs.isEmpty()
				&& previousPairs.get(previousPairs.size() - 1).mustBeLast()) {
			throw NotFoundException.getNotFoundInstance();
		}
	...
	DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern,
		...
		// Sanity check: element width for pattern and the character should
		// match
		float expectedElementWidth = (pattern.getStartEnd()[1] - pattern
				.getStartEnd()[0]) / 15.0f;
		if (Math.abs(elementWidth - expectedElementWidth)
				/ expectedElementWidth > 0.3f) {
			throw NotFoundException.getNotFoundInstance();
		}
