...
public final class Code128Reader extends OneDReader {
	...
	public Result decodeRow(int rowNumber, BitArray row,
		...
		boolean convertFNC1 = hints != null
					...
					if (code < 64) {
						...
						switch (code) {
							case CODE_FNC_1:
								if (convertFNC1) {
									if (result.length() == 0) {
										// GS1 specification 5.4.3.7. and
										// 5.4.6.4. If the first char after the
										// start code
										// is FNC1 then this is GS1-128. We add
										// the symbology identifier.
										result.append("]C1");
									} else {
										// GS1 specification 5.4.7.5. Every
										// subsequent FNC1 is returned as ASCII
										// 29 (GS)
		...
		if (!row.isRange(nextStart, Math.min(row.getSize(), nextStart
				+ (nextStart - lastStart) / 2), false)) {
			throw NotFoundException.getNotFoundInstance();
		}
		...
		return new Result(result.toString(), rawBytes, new ResultPoint[] {
